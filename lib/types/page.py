...
class Page:
	def __init__(self, img:cv2.typing.MatLike, mocr:manga_ocr.MangaOcr, file:str) -> None:
		self.file:str = file
	...
	def createMask(self, img:cv2.typing.MatLike) -> cv2.typing.MatLike:
		threshed:cv2.typing.MatLike = cv2.threshold(cv2.medianBlur(img, 5), 200, 255, cv2.THRESH_BINARY)[1] # type: ignore // reduced threshold less black
		morphed:cv2.typing.MatLike = cv2.morphologyEx(threshed, cv2.MORPH_RECT, cv2.getStructuringElement(cv2.MORPH_RECT,(7,7))) # type: ignore // (9,9) < -> less thick > -> thicker
		contours:list[cv2.typing.MatLike] = cv2.findContours(threshed, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)[0] # type: ignore
		filteredContours:list[cv2.typing.MatLike] = [contour for contour in contours if cv2.contourArea(contour) > 15000 and cv2.mean(morphed, mask=cv2.bitwise_not(morphed))[0] == 0]
	...
	def conjectBubbles(self) -> list[Bubble]:
		...
		blurred:cv2.typing.MatLike = cv2.medianBlur(gray, 3) # type: ignore
		threshed:cv2.typing.MatLike = cv2.threshold(blurred, 170, 255, cv2.THRESH_BINARY)[1] # type: ignore
		...
		scan = DBSCAN(eps=0.035*self.width, min_samples=2, algorithm="auto")
		...
		for l in np.unique(labels): # type: ignore
			...
			if not 1000 < bubbleBounds[2]*bubbleBounds[3] < 100000: continue
			bbs.append(Bubble(self.img, CropBox(*bubbleBounds,tolerance=20), self.mocr))
		return bbs
